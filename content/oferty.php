...
<div class="spady" style="min-height: 200px;">
...
<select name="typ_daty" id="typ_daty" class="testsel">
    ...
    <label for="tranzacja">Data dodania oferty</label>
    <div>
...
<?php      if($_SESSION['user']['type'] != 0){  ?>
         ...
         Dodawanie ofert: <strong> <?php if($klienci_glowna['oferty'] ==1 && $klienci_glowna['aktywny']==1 ){ echo 'aktywne' ; } else { echo '<font color="red"> nie aktywne</font>' ; }?> </strong>       
         
         
<?php if($klienci_glowna['oferty'] ==1 && $klienci_glowna['aktywny']== 1 ){ ?>   
<button class="btn"  onClick="location.href='?page=oferty&oferty_add=2'">dodaj ofertę</button>
<?php } else  {  ?>  <br><?php } } else { ?>         
Aby dodać ogłoszenie jako administator wybierz konto.<br>
&nbsp;
<?php } ?>
                 </div> 	
   
<form  <?php if($_SESSION['user']['type'] != 0){ ?> action="?page=<?php echo $_GET['page']; ?>&klient=1"<?php  } else { ?> action="?page=oferty&oferty_add=2"  <?php } ?>method="post">	
 <?php if($_SESSION['user']['type'] == 0){ echo  '<label class="control-label" for="konto">wybierz konto z którego dodasz ogłoszenie   :</label>';  } 
...
<?php 
 ...
 <?php } else { ?>
 <br>
 <input style="margin-top: 15px;" class="btn" type="submit" value="dodaj ofertę"> 
 <?php } ?>
</form>
 </div>
