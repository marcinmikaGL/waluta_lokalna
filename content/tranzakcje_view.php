<div style="position: absolute; top:-10px; left: 2%; right: 2%; z-index: 2000;">
<!-- tranzakcje_view.php -->
    ...
    <div class="modal-content">
      ...
      <div class="modal-body" style="height: 790px; padding: 5px; overflow-y: scroll;">
        <?php 
...
<div class="title-section"><div class="title"><p>Historia transakcji klienta</p></div></div> 
     ...
     <th id="nrSort" class="sort" onclick="changeSort('nr',1)" title="Sortowanie po: numer">NIP<span id="nrKier"></span></th>   
        ...
        <tbody> 
 <?php            
$tranzakcje_view = tranzakcje3($_GET['ids']);
...
?>
 ...
 <?php echo $klient_b['nazwa'];  ?> 
    ...
    <td><?php if($klient_b['typ_konta'] == 0){ echo $klient_b['nip']; } else { echo '[konto prywatne]';}; ?></td>
     ...
     <td><?php 
      if($tranzakcje_view[$i]['faktura_id_a'] != 1){
      $prosum2 +=  $tranzakcje_view[$i]['prowizja_a'] ; echo  number_format($tranzakcje_view[$i]['prowizja_a'],2) ;}
      else {
      echo 'FK ('.number_format($tranzakcje_view[$i]['prowizja_a'],2).')';
      }
      ?></td>
...
?>
 ...
 <?php echo $klient_b['nazwa'];  ?> 
    ...
    <td><?php if($klient_b['typ_konta'] == 0){ echo $klient_b['nip']; } else { echo '[konto prywatne]';}; ?></td>
     <td><?php echo user($tranzakcje_view[$i]['user_id']); ?></td>
      <td><?php 
      if($tranzakcje_view[$i]['faktura_id_b'] != 1){
      $prosum2 +=  $tranzakcje_view[$i]['prowizja_b'] ; echo  number_format($tranzakcje_view[$i]['prowizja_b'],2) ;}
      else {
      echo 'FK ('.number_format($tranzakcje_view[$i]['prowizja_b'],2).')';
      }
      ?></td>
  <td>-</td>
